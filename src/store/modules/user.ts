...
export default {
  ...
  state: () => ({ 
    ...
    nftAddress: "", // NFT contract address (can also be soulbound contract address)
  ...
  actions: { 
    ...
    async fetchCanUserBuy({ dispatch, commit, rootGetters }) {
    ...
    async fetchTokenData({ commit, rootGetters }) {
      if (address.value) {
        const intfc = new ethers.utils.Interface([
          "function allowance(address owner, address spender) public view returns (uint256)",
          "function balanceOf(address account) public view returns (uint256)"
        ]);
        const contract = new ethers.Contract(rootGetters["user/getPaymentTokenAddress"], intfc, signer.value);
        ...
        commit("setPaymentTokenAllowance", ethers.utils.formatEther(allowanceWei));
        commit("setPaymentTokenBalance", ethers.utils.formatEther(balanceWei));
